#Run 11:
#like run 8 except adding time-varying selectivity for VAST indices
...
parnms_use <- grep("mu|selpars_re", parnms, invert = T, value = T) #all but those for movement
...
tfit <- fit_wham(temp, do.retro=F, do.osa=F, do.sdrep =F)
saveRDS(tfit,here("2023.RT.Runs","Run11","phase1.RDS"))
#tfit <- readRDS(here("2023.RT.Runs","Run11","phase1.RDS"))

#phase 2, estimate q, logit_selpars
temp <- prepare_wham_input(asap, selectivity = sel, NAA_re = NAA_re, basic_info = basic_info, move = move, age_comp = "logistic-normal-miss0")
temp$fleet_names = paste0(rep(c("North_", "South_"),each = 2), temp$fleet_names)
temp$index_names = paste0(rep(c("North_", "South_"),c(3,3)), temp$index_names)
temp$map$log_index_sig_scale <- factor(rep(NA,length(temp$par$log_index_sig_scale)))
...
saveRDS(tfit2,here("2023.RT.Runs","Run11","phase2.RDS"))


#phase 3, estimate CV for rec CPA?
temp <- prepare_wham_input(asap, selectivity = sel, NAA_re = NAA_re, basic_info = basic_info, move = move, age_comp = "logistic-normal-miss0")
temp$fleet_names = paste0(rep(c("North_", "South_"),each = 2), temp$fleet_names)
temp$index_names = paste0(rep(c("North_", "South_"),c(3,3)), temp$index_names)
#temp$map$log_index_sig_scale <- factor(rep(NA,length(temp$par$log_index_sig_scale)))
temp$map$log_index_sig_scale <- factor(c(1,NA, NA, 4,NA, NA))
temp$map$trans_mu <- factor(rep(NA,length(temp$par$trans_mu)))
#temp$par$log_index_sig_scale[] <- 0 #doesn't converge with larger CVs
temp$map$log_NAA_sigma <- factor(rep(NA,length(temp$par$log_NAA_sigma)))
...
#Doesn't converge. CV for north rec CPA goes to 0.

#phase 3, estimate NAA sigma?
temp <- prepare_wham_input(asap, selectivity = sel, NAA_re = NAA_re, basic_info = basic_info, move = move, age_comp = "logistic-normal-miss0")
temp$fleet_names = paste0(rep(c("North_", "South_"),each = 2), temp$fleet_names)
temp$index_names = paste0(rep(c("North_", "South_"),c(3,3)), temp$index_names)
#temp$map$log_index_sig_scale <- factor(rep(NA,length(temp$par$log_index_sig_scale)))
...
tfit3 <- fit_wham(temp, do.retro=F, do.osa=F, do.sdrep =F)
#coverges nicely
saveRDS(tfit3,here("2023.RT.Runs","Run11","phase3.RDS"))

#phase 4, estimate NAA north 2+ S sigma?
temp <- prepare_wham_input(asap, selectivity = sel, NAA_re = NAA_re, basic_info = basic_info, move = move, age_comp = "logistic-normal-miss0")
temp$fleet_names = paste0(rep(c("North_", "South_"),each = 2), temp$fleet_names)
temp$index_names = paste0(rep(c("North_", "South_"),c(3,3)), temp$index_names)
temp$map$trans_mu <- factor(rep(NA,length(temp$par$trans_mu)))
...
mohns_rho(fit)
setwd(here("2023.RT.Runs","Run11"))
Run8 <- readRDS(here("2023.RT.Runs","Run8","fit.RDS"))
Run8$opt$obj + length(Run8$opt$par) #compare 0.5*AIC
fit$opt$obj + length(fit$opt$par)
saveRDS(fit,"fit.RDS")
...
#jitter
library("mvtnorm")
fit_file <-here("2023.RT.Runs","Run11","fit.RDS")
fit <- readRDS(here("2023.RT.Runs","Run11","fit.RDS"))
...
saveRDS(jit_res, here("2023.RT.Runs","Run11","jitter_res.RDS"))
...
saveRDS(x, here("2023.RT.Runs","Run11","jitter_res.RDS"))

sort(sapply(x, function(y) y$obj))
temp <- 1+ (sapply(x, function(y) y$max_gr) < 5)
difnll <- sapply(x, function(y) y$obj) - fit$opt$obj 
png(here("2023.RT.Runs","Run11","jitter_results.png"), width=7, height=7, units='in', res = 300)
...
saveRDS(sim_res, here("2023.RT.Runs","Run11","self_test_res.RDS"))
