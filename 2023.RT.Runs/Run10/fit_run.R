...
Run9 <- readRDS(here("2023.RT.Runs","Run9","fit.RDS"))
parnms <- sort(names(Run9$parList))
...
temp$par[parnms_use] <- Run9$parList[parnms_use]
...
#try D-M age comp
asap_alt <- asap
asap_alt[[1]]$dat$IAA_mats <- lapply(asap[[1]]$dat$IAA_mats, function(x) {
	x[which(x[,12]>0),12] <- 1000
	return(x)
})
asap_alt[[2]]$dat$IAA_mats <- lapply(asap[[2]]$dat$IAA_mats, function(x) {
	x[which(x[,12]>0),12] <- 1000
	return(x)
})
asap_alt[[1]]$dat$catch_Neff[which(asap_alt[[1]]$dat$catch_Neff[]>0)] <- 1000
asap_alt[[2]]$dat$catch_Neff[which(asap_alt[[2]]$dat$catch_Neff[]>0)] <- 1000
...
tfit$parList$catch_paa_pars
sapply(1:4, function(x) 1 + (temp$data$catch_Neff[,x] -1)/(1 + temp$data$catch_Neff[,x] * exp(-tfit$parList$catch_paa_pars[x,1])))
sapply(1:6, function(x) 1 + (temp$data$index_Neff[,x] -1)/(1 + temp$data$index_Neff[,x] * exp(-tfit$parList$index_paa_pars[x,1])))
#Scale in north is order of magnitute higher. 

#back to multinomial to start, then use L-N
temp <- prepare_wham_input(asap, selectivity = sel, NAA_re = NAA_re, basic_info = basic_info, move = move)
temp$fleet_names = paste0(rep(c("North_", "South_"),each = 2), temp$fleet_names)
temp$index_names = paste0(rep(c("North_", "South_"),c(3,3)), temp$index_names)
tfit_multinom <- fit_wham(temp, do.retro=F, do.osa=F, do.sdrep =F)

temp <- prepare_wham_input(asap, selectivity = sel, NAA_re = NAA_re, basic_info = basic_info, move = move, age_comp = "logistic-normal-miss0")
temp$fleet_names = paste0(rep(c("North_", "South_"),each = 2), temp$fleet_names)
temp$index_names = paste0(rep(c("North_", "South_"),c(3,3)), temp$index_names)
parnms <- sort(names(tfit_multinom$parList))
#t(sapply(parnms, function(x) c(length(Run7$parList[[x]]), length(temp$par[[x]]))))
parnms_use <- grep("logit_selpars", parnms, invert = T, value = T) #all but those for selectivity
temp$par[parnms_use] <- Run9$parList[parnms_use]
