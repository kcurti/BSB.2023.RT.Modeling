...
ecov$logsigma <- log(cbind(north_bt[,'sd'], south_bt[,'sd']))
...
#ecov$recruitment_how <- matrix(c("controlling-lag-1-linear","none","none","controlling-lag-1-linear") 2,2)

temp <- prepare_wham_input(asap_alt, ecov = ecov)
...
x <- names(temp$map)
sapply(x, function(x) class(temp$map[[x]]))
cbind(sapply(x, function(x) length(temp$map[[x]])), sapply(x, function(x) length(temp$par[[x]])))
