...
run.dir <- file.path("2023.RT.Runs","Run6.FixedData")

asap <- read_asap3_dat(c(file.path(run.dir, "NORTH.FIXED.DATA.RUN.1.DAT"), file.path(run.dir, "SOUTH.FIXED.DATA.RUN.1.DAT")))
...
sel$initial_pars <- c(rep(list(c(5,1)),8), list( 
	### begin north
	c(rep(c(0.5,1,0.5), c(1,2,5))), #rec cpa age-specfic (9) 
	...
	c(rep(c(0.5,1,0.5), c(1,1,6)))) #VAST fall age-specific (14)
)
sel$fix_pars <- c(rep(list(NULL),8), list(
	###begin north
  2:3, #rec cpa
