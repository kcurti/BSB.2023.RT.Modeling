...
run.dir <- file.path("2023.RT.Runs","Run8.FixedData")
...
Run7 <- readRDS(file.path("2023.RT.Runs","Run7.FixedData","fit.RDS"))
...
saveRDS(tfit,file.path("2023.RT.Runs","Run8.FixedData","phase1.RDS"))
...
sel$initial_pars <- c(rep(list(c(5,1)),4), list(rep(c(0.5,1),c(1,7))), rep(list(c(5,1)),3), list( 
	### begin north
	c(rep(c(0.5,1,0.5), c(2,4,2))), #rec cpa age-specfic (9)
	c(rep(c(0.5,1,0.5), c(2,2,4))), #VAST spring age-specfic (10)
	c(rep(c(0.5,1,0.5), c(3,1,4))),#) #VAST fall age-specfic (11)
	####end north, begin south
	c(rep(c(0.5,1,0.5,0.5), c(2,1,4,1))), #rec cpa age-specific (12)
	c(rep(c(0.5,1,0.5), c(2,1,5))), #VAST spring age-specific
	c(rep(c(1,0.5,0.5), c(1,1,6)))) #VAST fall age-specific
)
sel$fix_pars <- c(rep(list(NULL),4), list(2:8), rep(list(NULL),3), list(
	###begin north
  3:6, #rec cpa
...
tfit2 <- fit_wham(temp, do.retro=F, do.osa=F, do.sdrep =T)   ##### Did not converge
saveRDS(tfit2,file.path("2023.RT.Runs","Run8.FixedData","phase2.RDS"))
...
#jitter
library("mvtnorm")
fit_file <-here("2023.RT.Runs","Run8.FixedData","fit.RDS")
fit <- readRDS(here("2023.RT.Runs","Run8.FixedData","fit.RDS"))
...
saveRDS(x, here("2023.RT.Runs","Run8.FixedData","jitter_res.RDS"))

sort(sapply(x, function(y) y$obj))
temp <- 1+ (sapply(x, function(y) y$max_gr) < 5)
difnll <- sapply(x, function(y) y$obj) - fit$opt$obj 
png(here("2023.RT.Runs","Run8.FixedData","jitter_results.png"), width=7, height=7, units='in', res = 300)
...
saveRDS(sim_res, here("2023.RT.Runs","Run8.FixedData","self_test_res.RDS"))
