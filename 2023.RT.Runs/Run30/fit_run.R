...
temp$par$log_NAA_sigma[1,2,2:8] <- log(0.05)
tfit <- fit_wham(temp, do.fit =F)
#lapply(tfit$rep[grep("nll", names(tfit$rep))], sum)

tfit <- fit_wham(temp, do.sdrep =F, do.osa =F, do.retro = F)
saveRDS(tfit, here("2023.RT.Runs", "Run30", "tfit.RDS"))
Run29 <- readRDS(here("2023.RT.Runs","Run29", "fit.RDS"))
...
saveRDS(tfit2, here("2023.RT.Runs", "Run30", "tfit2.RDS"))
tfit2$parList$logit_selpars
#need to make south vast flat topped at age 2
...
mohns_rho(fit)
saveRDS(fit, here("2023.RT.Runs","Run30", "fit.RDS"))
setwd(here("2023.RT.Runs","Run30"))
...
fit_file <-here("2023.RT.Runs","Run30","fit.RDS")
set.seed(8675309)
...
saveRDS(sim_res_all, here::here("2023.RT.Runs","Run30","self_test_res.RDS"))
