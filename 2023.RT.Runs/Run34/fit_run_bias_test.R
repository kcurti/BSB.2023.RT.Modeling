...
#                    bias_correct_process = FALSE,bias_correct_observation=FALSE,bias_correct_BRPs=FALSE)
basic_info <- list(region_names = c("North", "South"), stock_names = paste0("BSB_", c("North", "South")),
                   bias_correct_process = TRUE,bias_correct_observation=TRUE,bias_correct_BRPs=TRUE)
...
# BIAS.NO <- fit_wham(temp, do.sdrep = T, do.osa = T, do.retro = T, do.brps = T)
BIAS.YES <- fit_wham(temp, do.sdrep = T, do.osa = T, do.retro = T, do.brps = T)
# mods <- list(BIAS.NO=BIAS.NO,BIAS.YES=BIAS.YES)
# compare_wham_models(mods=mods)

# mohns_rho(fit)
saveRDS(BIAS.YES, here("2023.RT.Runs",this_run, "BC.fit.RDS"))
setwd(here("2023.RT.Runs",this_run,"Bias Correction"))
plot_wham_output(BIAS.YES)
