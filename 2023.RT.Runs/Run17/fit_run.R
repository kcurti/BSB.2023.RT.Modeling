#Run 16:
#like Run 16 but change to dirichlet-miss-0
...
#north fall vast selectivity is going to 1 for older ages. make it flat topped
saveRDS(tfit1,here("2023.RT.Runs","Run17","phase1.RDS"))
...
sel$initial_pars <- c(
	...
	rep(list(c(5,1)),4), #rest of southern fleets blocks then north rec cpa
	list(rep(c(0.5,1,0.5),c(2,2,4))), #north spring vast 
	list(rep(c(0.5,1),c(2,6))), #north fall vast 
	rep(list(c(5,1)),1), #south rec cpa
	list(rep(c(0.5,1,0.5),c(1,1,6))), #south spring vast 
	list(rep(c(0.5,1,0.5),c(1,1,6))) #south fall vast 
)
...
#now south rec cpa CV is going to 0


temp$map$log_index_sig_scale <- factor(c(1,NA, NA, NA,NA, NA)) #try to estimate CVs
tfit_Dir <- fit_wham(temp, do.retro=F, do.osa=F, do.sdrep =F)
tfit2 <- tfit_Dir
saveRDS(tfit2,here("2023.RT.Runs","Run17","phase2.RDS"))
#now selblock 12 (south rec cpa) needs to go to flat-topped
...
saveRDS(tfit3,here("2023.RT.Runs","Run17","phase3.RDS"))
#now selblock 9 (north rec cpa) needs to go to flat-topped
...
saveRDS(tfit4,here("2023.RT.Runs","Run17","phase4.RDS"))
