...
FAA.fleet <- WHAM_output$rep$FAA
  dimnames(FAA.fleet)[[1]] <- input$fleet_names
  dimnames(FAA.fleet)[[2]] <- as.character(model.yrs)
  dimnames(FAA.fleet)[[3]] <- ages.labels
...
dim(WHAM_output.ests$log_NAA_rep)       # 2 x 2 x 33 x 8
dim(WHAM_output.ests$log_SSB)           # 33 x 2
dim(WHAM_output.ests$log_SSB_all)       # 33
...
mohns.rho.table <- tibble(Run=rep(run.no, 2), Region=c('North','South'), Fmort=round(mohns.rho$Fbar,3), SSB=round(mohns.rho$SSB,3), Rect=round(c(mohns.rho$naa[1,1,'R'],mohns.rho$naa[2,2,'R']),3))
# View(mohns.rho.table)


nll <- WHAM_output$opt$obj
nparam <- length(WHAM_output$opt$par)
AIC <- 2*(WHAM_output$opt$obj + length(WHAM_output$opt$par))
model.fit.table <- tibble(Run=run.no, NLL=nll, nparam=nparam, AIC=AIC)
...
save.image(file.path(output.dir, paste(run.no,"WHAM.Outputs.RDATA",sep='.')))
write.csv(mohns.rho.table, file.path(output.dir, paste(run.no, "Mohns.Rho.values.csv", sep='.')), row.names = FALSE)
write.csv(model.fit.table, file.path(output.dir, paste(run.no, "model.fit.table.csv", sep='.')), row.names = FALSE)
for (reg in c("north","south"))
{
  write.csv(get(paste("SSB.yr",reg,sep='.')), file.path(output.dir, paste(run.no,reg, "SSB.yr.csv", sep='.')), row.names = FALSE)
  write.csv(get(paste("F.yr",reg,sep='.')), file.path(output.dir, paste(run.no,reg, "F.yr.csv", sep='.')), row.names = FALSE)
  write.csv(get(paste("Rect.yr",reg,sep='.')), file.path(output.dir, paste(run.no,reg, "Rect.yr.csv", sep='.')), row.names = FALSE)
}
for (flt in dimnames(FAA.fleet)[[1]])
{
  write.csv(FAA.fleet[flt,,], file.path(output.dir, paste(run.no, flt, "FAA.csv", sep='.')), row.names = TRUE)
}
