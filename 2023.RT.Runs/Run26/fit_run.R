#Run 26:
#like Run 24, but round age comp to two digits
...
round_age_comp_fn <- function(asap, digits = 2){
	for(i in 1:length(asap)) {
		asap[[i]]$dat$IAA_mats <- lapply(asap[[i]]$dat$IAA_mats, function(x) {
			cols <- 4:(NCOL(x)-1)
			temp <- t(apply(x[,cols],1,round,digits))
			print(dim(temp))
			temp <- t(apply(temp,1,function(y) y/sum(y)))
			print(dim(temp))
			x[,cols] <- temp
			return(x)
		})
...
asap_alt <- round_age_comp_fn(asap_alt)
