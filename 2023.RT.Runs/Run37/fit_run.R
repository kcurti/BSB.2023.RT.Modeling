...
library(here)
heretype <- "short" #which type of here did it choose to do today
# asap <- read_asap3_dat(c(here("Bridge.runs", "Run9", "NORTH.RUN.9.DAT"),here("Bridge.runs", "Run9", "SOUTH.RUN.9.DAT")))
if(heretype=='short') asap <- read_asap3_dat(c(here("Bridge.runs", "Run7", "NORTH.BRIDGE.RUN.7.DAT"),here("Bridge.runs", "Run7", "SOUTH.BRIDGE.RUN.7.DAT")))
if(heretype=='long') asap <- read_asap3_dat(c(here("GitHub","BSB.2023.RT.Modeling","Bridge.runs", "Run7", "NORTH.BRIDGE.RUN.7.DAT"),here("GitHub","BSB.2023.RT.Modeling","Bridge.runs", "Run7", "SOUTH.BRIDGE.RUN.7.DAT")))
...
fit10 <-readRDS("fit10.RDS")
#End Model 10 ---------------------------

#Model 11 ---------------------------
#Same as model 10 but with random effects in selectivity on: BTS
#With: environmental covariates, movement, random effects
#Catch still has random effects on selectivity
# for(i in c(1:6,8:15,17)) asap_alt$dat$IAA_mats[[i]] <- asap$dat$IAA_mats[[i]]
# north_bt <- read.csv(here("2023.RT.Runs","Run33","bsb_bt_temp-nmab.csv"))
...
# sel$re <- rep(c("2dar1","2dar1","none","none","none"), c(1,1,2+4+6,1,10))
sel$re <- rep(c("2dar1","2dar1","none","ar1_y","none","ar1_y","none"), c(1,1,2+4,1,5,2,9))
#ar1_y on north rec cpa
temp <- prepare_wham_input(asap_alt, selectivity = sel, NAA_re = NAA_re, basic_info = basic_info, move = move, ecov = ecov,
                           age_comp = list(
                             fleets = c("dir-mult","logistic-normal-miss0","logistic-normal-ar1-miss0","logistic-normal-ar1-miss0"), 
...
# fit11 <-readRDS("fit11.RDS")
#End Model 11 ---------------------------

#Model 12 ---------------------------
#Same as model 10 but with random effects in selectivity on: BTS, NEAMAP
#With: environmental covariates, movement, random effects
#Catch still has random effects on selectivity
# for(i in c(1:6,8:15,17)) asap_alt$dat$IAA_mats[[i]] <- asap$dat$IAA_mats[[i]]
# north_bt <- read.csv(here("2023.RT.Runs","Run33","bsb_bt_temp-nmab.csv"))
...
# sel$re <- rep(c("2dar1","2dar1","none","none","none"), c(1,1,2+4+6,1,10))
sel$re <- rep(c("2dar1","2dar1","none","ar1_y","none","ar1_y","none"), c(1,1,2+4,2,4,2,9))
#ar1_y on north rec cpa
temp <- prepare_wham_input(asap_alt, selectivity = sel, NAA_re = NAA_re, basic_info = basic_info, move = move, ecov = ecov,
                           age_comp = list(
                             fleets = c("dir-mult","logistic-normal-miss0","logistic-normal-ar1-miss0","logistic-normal-ar1-miss0"), 
...
# fit12 <-readRDS("fit12.RDS")
#End Model 12 ---------------------------

#Model 13 ---------------------------
#Same as model 10 but with random effects in selectivity on: BTS, NEAMAP, State surveys in North (-NY)
#With: environmental covariates, movement, random effects
#Catch still has random effects on selectivity
# for(i in c(1:6,8:15,17)) asap_alt$dat$IAA_mats[[i]] <- asap$dat$IAA_mats[[i]]
# north_bt <- read.csv(here("2023.RT.Runs","Run33","bsb_bt_temp-nmab.csv"))
...
# sel$re <- rep(c("2dar1","2dar1","none","none","none"), c(1,1,2+4+6,1,10))
sel$re <- rep(c("2dar1","2dar1","none","ar1_y","none","ar1_y","none"), c(1,1,2+4,5,1,2,9))
#ar1_y on north rec cpa
temp <- prepare_wham_input(asap_alt, selectivity = sel, NAA_re = NAA_re, basic_info = basic_info, move = move, ecov = ecov,
                           age_comp = list(
                             fleets = c("dir-mult","logistic-normal-miss0","logistic-normal-ar1-miss0","logistic-normal-ar1-miss0"), 
...
# fit13 <-readRDS("fit13.RDS")
#End Model 13 ---------------------------

#Model 14 ---------------------------
#Same as model 10 but with random effects in selectivity on: BTS, NEAMAP, State surveys (-NY)
#Selectivity RE is 2dar1 on BTS, NEAMAP
#With: environmental covariates, movement, random effects
#Catch still has random effects on selectivity
# for(i in c(1:6,8:15,17)) asap_alt$dat$IAA_mats[[i]] <- asap$dat$IAA_mats[[i]]
# north_bt <- read.csv(here("2023.RT.Runs","Run33","bsb_bt_temp-nmab.csv"))
...
# Run37.11 <- readRDS(here("2023.RT.Runs",this_run, "fit11.RDS"))
# Run37.12 <- readRDS(here("2023.RT.Runs",this_run, "fit12.RDS"))
# Run37.13 <- readRDS(here("2023.RT.Runs",this_run, "fit13.RDS"))
Run37.14 <- readRDS(here("2023.RT.Runs",this_run, "fit14.RDS"))
Run37.14i <- readRDS(here("2023.RT.Runs",this_run, "fit14i.RDS"))
Run37.14ii <- readRDS(here("2023.RT.Runs",this_run, "fit14ii.RDS"))
Run37.14iii <- readRDS(here("2023.RT.Runs",this_run, "fit14iii.RDS"))
}
if(heretype=='long'){
  # Run34 <- readRDS(here("2023.RT.Runs","Run34", "fit.RDS"))
  # Run37.10 <- readRDS(here("2023.RT.Runs",this_run, "fit10.RDS"))
  ...
  # Run37.12 <- readRDS(here("2023.RT.Runs",this_run, "fit12.RDS"))
  # Run37.13 <- readRDS(here("2023.RT.Runs",this_run, "fit13.RDS"))
  Run37.14 <- readRDS(here("GitHub","BSB.2023.RT.Modeling","2023.RT.Runs",this_run, "fit14.RDS"))
  Run37.14i <- readRDS(here("GitHub","BSB.2023.RT.Modeling","2023.RT.Runs",this_run, "fit14i.RDS"))
  Run37.14ii <- readRDS(here("GitHub","BSB.2023.RT.Modeling","2023.RT.Runs",this_run, "fit14ii.RDS"))
  Run37.14iii <- readRDS(here("GitHub","BSB.2023.RT.Modeling","2023.RT.Runs",this_run, "fit14iii.RDS"))
}
# mods <- list(Run34=Run34,Run37.10=Run37.10,Run37.11=Run37.11,Run37.12=Run37.12,Run37.13=Run37.13)
mods <- list(Base=Run37.14,increase125=Run37.14i,increase150=Run37.14ii,increase175=Run37.14iii)
compare_wham_models(mods=mods,plot.opts = list(which=c(1,2,3,5,6,7,8,9,10)))
#End Compare Model Objects

#Model 14.i ---------------------------
#Changing the weight of MA Index
#MA CV: 125%
CVadjust <- 1.25
...
temp$data$agg_index_sigma[,3] <- temp$data$agg_index_sigma[,3] * CVadjust
# indices = rep(c("dir-mult","logistic-normal-miss0","dir-mult","logistic-normal-ar1-miss0","dir-mult"),c(6,1,8,1,1))))
temp$fleet_names = paste0(rep(c("North_", "South_"),each = 2), temp$fleet_names)
temp$index_names = paste0(rep(c("North_", "South_"),c(8,9)), temp$index_names)
temp$map$trans_mu <- factor(rep(NA,length(temp$par$trans_mu)))
...
# tfit14i <- fit_wham(temp, do.sdrep = F, do.osa = F, do.retro = F)
# saveandplot(tfit14i,"tfit14i")

fit14i <- fit_wham(temp, do.sdrep = T, do.osa = T, do.retro = T, do.brps = T)
saveandplot(fit14i,"fit14i")

# if(heretype=='long') fit14 <-readRDS(here("GitHub","BSB.2023.RT.Modeling","2023.RT.Runs","Run37", "fit14.RDS"))
# plot_wham_output(fit14)
...
fit14ii <- fit_wham(temp, do.sdrep = T, do.osa = T, do.retro = T, do.brps = T)
saveandplot(fit14ii,"fit14ii")
...
fit14iii <- fit_wham(temp, do.sdrep = T, do.osa = T, do.retro = T, do.brps = T)
saveandplot(fit14iii,"fit14iii")
