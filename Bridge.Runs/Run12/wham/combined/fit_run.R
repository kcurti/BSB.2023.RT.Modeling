...
mohns_rho(fit)
setwd(here("Bridge.runs","Run12", "wham","combined"))
run11 <- readRDS(here("Bridge.runs","Run11", "wham","combined", "fit.RDS"))
run11$opt$obj + length(run11$opt$par)
fit$opt$obj + length(fit$opt$par) #run11 AIC is better 
