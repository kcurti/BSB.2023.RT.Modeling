...
#naa re back off to "rec" instead of "rec+1" to see if ar1 on move rate is better than "rec+1"
...
NAA_re = list(sigma = list("rec","rec"), cor = list("iid","iid"), N1_model = rep("equilibrium",2))
...
fit$opt$par #some sel pars hitting bounds
fit$opt$obj + length(fit$opt$par)
run14 <- readRDS(here("Bridge.runs","Run14", "wham","combined", "fit.RDS"))
run14$opt$obj + length(run$opt$par)
#probably not worth it to pursue because aic is much worse than rec+1

#mohns_rho(fit)
setwd(here("Bridge.runs","Run17", "wham","combined"))
saveRDS(fit,"fit.RDS")
setwd(here())
