...
#ar1(year) on north-south movement for north pop. south-north remains constant.
...
fit$sdrep #doesn't converge

#try redoing age at max selectivity for the vast indices
#also, rec cpa paa is different from rec catch paa so why would the selectivity be the same?
sel <- list(model = rep(c("logistic","age-specific"),c(8,4)))
sel$initial_pars <- c(
	rep(list(c(5,1)),8), 
	list( 
	c(rep(c(0.5,0.5), c(7,1))), #north rec cpa 
	c(rep(c(0.5,0.5,0.5), c(2,4,2))), #north vast spring
	c(rep(c(0.5,0.5,0.5), c(2,1,5))), #south rec cpa
	c(rep(c(0.5,0.5,0.5), c(1,2,5)))) #south vast spring
)
sel$fix_pars <- c(
	rep(list(NULL),8), 
	list(
...
input <-prepare_wham_input(asap, basic_info = basic_info, selectivity = sel, move = move, NAA_re = NAA_re, age_comp = "logistic-normal-miss0")
#input$data$selblock_pointer_indices[,c(1,3)] <- input$data$selblock_pointer_fleets[,c(2,4)]
input$par$mu_repars[,,1,1,1] <- log(0.1)
input$map$mu_repars <- factor(rep(NA, length(input$par$mu_repars)))
input$par$mu_repars[,,1,1,1] <- log(0.1)


fit <- fit_wham(input, do.retro = F, do.osa = F, do.sdrep=F)
#still crashes.

run14 <- readRDS(here("Bridge.runs","Run14", "wham","combined", "fit.RDS"))
input$par <- run14$parList
remove(run14)
#start at run 14 estimates
fit <- fit_wham(input, do.retro = F, do.osa = F)
...
# mohns_rho(fit)
# setwd(here("Bridge.runs","Run15", "wham","combined"))
# saveRDS(fit,"fit.RDS")
