...
Viewing the bivariate estimate of stock and harvest status as a posterior distribution, the current status is 0.59 probability of \(F>F_{40}\) and SSB \$\textgreater{}\(0.5\times SSB\)(F\_\{40\})\$ and 0.41 probability of \(F<F_{40}\) and SSB\(>0.5\times\) SSB\((F_{40})\), with negligible probability of SSB\(<0.5\times\) SSB\((F_{40})\) (Figure \ref{fig:kobe-status}).
