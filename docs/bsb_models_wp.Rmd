...
Given the revised model with the lower marginal NLL, we used the same jittering method with 100 simulated parameter values as starting values (Figure XX). None of these iterations resulted in lower marginal NLLs than the new estimated values.
...
Nearly all models converged poorly which appeared to be attributable to the estimation of the scalar for the standard errors of the log transformed Rec CPA indices (Figure XX). At least one of these scalars nearly always tended to 0. However, even for these fits with poor convergence, the SSB estimates appeared to be reliable (Figure XX). When we redid the self-tests fixing those parameters at the estimated values, the convergence was much improved.
